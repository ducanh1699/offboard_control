...
// check the UAV reach target or not ?
...
//     double a = stop.norm();
//     if (a <= error){
//         cout << "In the acceptable range!" << endl;
//         cout << a << endl;
//         return true;
//     }
//     else
...
// check have the ids or not ?
bool check_ids;
void check_cb_ids(const std_msgs::Bool msg){
    check_ids = msg.data;
}

// input the positions that you want to fly
...
// check move to the centre of UAV
bool check_mov;
void check_cb(const std_msgs::Bool msg){
    check_mov = msg.data;
}

// call back the marker position 
...
{   
    ...
    {   
        ...
        for (i = 1; i < n; i++)
        {
            ...
            while (!check_target && ros::ok())
            {   
                a.twist.linear.x = pid_cmd.compute(set_point.pose.position.x, current_position.pose.position.x, last_time);
                a.twist.linear.y = pid_cmd1.compute(set_point.pose.position.y, current_position.pose.position.y, last_time);
                a.twist.linear.z = pid_cmd2.compute(set_point.pose.position.z, current_position.pose.position.z, last_time);
                cmd_vel.publish(a);
                ros::spinOnce();
                rate.sleep();
                check_target = check_position(error, current_position, set_point);
